...
def maybe_emit_signal(levels, params, state: AppState, bus, sheet, cfg):
    ...
    if (near(spot,s1) or near(spot,s2)) and can_fire("CE"):
        bus.emit("signal", {"side":"CE","level_hit":"S1" if near(spot,s1) else "S2","reason":f"Near support within {entry_band}","spot":spot})
        state.last_signal_ts["CE"] = now
    if (near(spot,r1) or near(spot,r2)) and can_fire("PE"):
        bus.emit("signal", {"side":"PE","level_hit":"R1" if near(spot,r1) else "R2","reason":f"Near resistance within {entry_band}","spot":spot})
        state.last_signal_ts["PE"] = now

def on_levels(levels, params, state, bus, sheet, cfg):
