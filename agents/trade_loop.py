...
def manage_open_trades():
    ...
    for tid, t in _open.items():
        ...
        if t.side == "CE":
            ...
            if ltp >= t.sl or ltp <= t.tp:
                to_close.append((tid, ltp))
    for tid, exit_ltp in to_close:
        t = _open[tid]
        t.status = "CLOSED"
        try:
            sh.append_row("Trades", [t.id, t.signal_id, t.side, t.entry, t.sl, t.tp, t.qty, f"CLOSED@{exit_ltp}"])
        except Exception as e:
            log.error(f"Trades close log failed: {e}")
        log.info(f"Trade closed {tid} @ {exit_ltp}")
        del _open[tid]
