...
def _parse_symbols() -> list[str]:
    ...
    if not raw: return ["NIFTY"]
    parts = [p.strip() for p in raw.split(",") if p.strip()]
    if not parts: return ["NIFTY"]
...
def _compute_levels_from_oc(oc_json: dict) -> dict:
    ...
    for k, v in chain.items():
        ...
        ce = (v or {}).get("ce") or {}; pe = (v or {}).get("pe") or {}
...
def update_levels_from_dhan(bus):
    client = DhanClient()
    symbols = _parse_symbols(); primary = _choose_primary(symbols)
    levels_all = {}
    for sym in symbols:
        try:
            usid = client.resolve_underlying_scrip(sym)
            if not usid: print(f"[oc] resolve fail: {sym}"); continue
            exps = client.get_expiries(usid)
            if not exps: print(f"[oc] no expiries: {sym}"); continue
            expiry = exps[0]
