...
# Shows header + shifted levels + C1..C6 ✓/❌ + summary.
# Provides register(application) to hook into your bot.
# ------------------------------------------------------------
...
# Prefer shim passthrough for snapshot
try:
    from analytics.oc_refresh_shim import get_snapshot  # forwards to analytics.oc_refresh.get_snapshot if present
except Exception:
    def get_snapshot():
        return None
...
async def cmd_oc_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    snap = get_snapshot()
    if snap is None:
        await update.message.reply_text("Waiting for first OC snapshot…")
        return
